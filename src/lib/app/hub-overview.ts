...
import type { WeeklyTicketPoint } from "@/lib/app/drawer-stats";
import type {
  HubActivityItem,
  HubOverviewData,
  HubRankedEntity,
...
import { getDashboardStats } from "@/lib/app/queries";
...
function buildWeeklyCompletionSeries(tickets: Ticket[]): WeeklyTicketPoint[] {
  return buildWeeklyTicketSeries(
    tickets,
    (ticket) => ticket.status === "completed",
    (ticket) =>
      ticket.activity?.find((entry) => entry.kind === "completion")?.createdAt ??
      ticket.updatedAt
  );
}
