...
const IDENTIFIER_FIELDS: Partial<Record<MockupCollectionSlug, string>> = {
...
export function getIdentifierField(slug: MockupCollectionSlug): string | null {
  return IDENTIFIER_FIELDS[slug] ?? null
}

export function getDocIdentifier(doc: Record<string, unknown>, slug: MockupCollectionSlug): string {
...
export function resolveIdentifierParam(param: string): { mode: 'id' | 'field'; value: string } {
...
export function getDocTitle(doc: Record<string, unknown>, slug: MockupCollectionSlug): string {
  switch (slug) {
    case 'employees':
