...
import {
  TicketDetailDrawer,
  TicketRowTrigger,
} from "@/components/mockups/mr/TicketDetailDrawer";
...
import { formatDate } from "@/lib/mockups/helpers";
import {
  formatDurationHours,
  type BreakdownItem,
...
import { MR_NAV } from "@/lib/mockups/mr/navigation";
import { cn } from "@/utils/cn";

type MrHubDashboardProps = {
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 border-stroke-soft-200 border-b px-5 py-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <LinkButton.Root asChild size="medium" variant="gray">
            <Link href="/mockups/mr/tickets">Details</Link>
          </LinkButton.Root>
        </div>
      </div>
  ...
  return (
    <ChartCard>
      <div className="flex flex-wrap items-start justify-between gap-4 border-stroke-soft-200 border-b px-5 py-4">
        ...
        <LinkButton.Root asChild size="medium" variant="gray">
          <Link href="/mockups/mr/tickets">View closed</Link>
        </LinkButton.Root>
      </div>
  ...
  return (
    <ChartCard>
      <div className="flex items-center justify-between gap-3 border-stroke-soft-200 border-b px-5 py-4">
        ...
        <LinkButton.Root asChild size="medium" variant="gray">
          <Link href="/mockups/mr/tickets">View reports</Link>
        </LinkButton.Root>
      </div>
  ...
  return (
    <ChartCard>
      <div className="flex items-center justify-between gap-3 border-stroke-soft-200 border-b px-5 py-4">
        ...
        <LinkButton.Root asChild size="medium" variant="gray">
          <Link href="/mockups/mr/assets">Details</Link>
        </LinkButton.Root>
      </div>
      <div className="space-y-4 px-5 py-4">
        <SegmentedProgressBar
  ...
  return (
    <ChartCard>
      <div className="flex items-center justify-between gap-3 border-stroke-soft-200 border-b px-5 py-4">
        ...
        <LinkButton.Root asChild size="medium" variant="gray">
          <Link href="/mockups/mr/locations">All locations</Link>
        </LinkButton.Root>
      </div>
        ...
        {items.length === 0 ? (
          ...
          items.map((item, index) => (
            <Link
              ...
              href={
                variant === "company"
                  ? `/mockups/mr/companies/${item.id}`
                  : "/mockups/mr/maintenance-teams"
              }
              key={item.id}
            >
  ...
  return (
    <ChartCard>
      <div className="flex items-center justify-between gap-3 border-stroke-soft-200 border-b px-5 py-4">
        ...
        <LinkButton.Root asChild size="medium" variant="gray">
          <Link href="/mockups/mr/maintenance-teams">All teams</Link>
        </LinkButton.Root>
      </div>
      <div className="space-y-4 px-5 py-4">
        {teams.length === 0 ? (
...
function QuickNavCard({ data }: { data: HubOverviewData }) {
  const navItems = MR_NAV.flatMap((group) => group.items).filter(
    (item) => item.href !== "/mockups/mr"
  );
...
function RecentActivitiesPanel({ data }: { data: HubOverviewData }) {
    ...
    <ChartCard>
      <div className="flex items-start justify-between gap-3 border-stroke-soft-200 border-b px-5 py-4">
        <div>
          ...
          <p className="mt-0.5 text-paragraph-xs text-text-sub-600">
            {data.activitiesToday} new{" "}
            {data.activitiesToday === 1 ? "activity" : "activities"} today
          </p>
        </div>
        <LinkButton.Root asChild size="medium" variant="gray">
          <Link href="/mockups/mr/tickets">Details</Link>
...
function RecentTicketsPanel({ data }: { data: HubOverviewData }) {
    ...
    <>
      <ChartCard>
        <div className="flex items-center justify-between gap-3 border-stroke-soft-200 border-b px-5 py-4">
          ...
          <LinkButton.Root asChild size="medium" variant="gray">
            <Link href="/mockups/mr/tickets">View all</Link>
          </LinkButton.Root>
        </div>
        <div className="divide-y divide-stroke-soft-200">
          {data.recentTickets.length === 0 ? (
            <p className="px-5 py-8 text-center text-paragraph-sm text-text-soft-400">
              No tickets yet
            </p>
          ) : (
...
export function MrHubDashboard({ data }: MrHubDashboardProps) {
  return (
